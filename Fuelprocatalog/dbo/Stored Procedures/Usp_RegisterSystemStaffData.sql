...
BEGIN
   BEGIN
	...
	BEGIN
		...
		BEGIN
			IF EXISTS(SELECT Userid FROM Tenantusers WHERE Username=JSON_VALUE(@JsonObjectdata, '$.Username'))
			BEGIN
				Select  1 as RespStatus, 'Similar username exists!. Contact Admin!' as RespMessage;
				Return
			END
			IF EXISTS(SELECT Userid FROM Tenantusers WHERE Phonenumber=JSON_VALUE(@JsonObjectdata, '$.Phonenumber'))
			BEGIN
		...
		BEGIN
		  ...
		  SELECT @Data1 = a.Userid,@Data2=a.Firstname +' '+ a.Lastname,@Data4=Passwords,@Data5=Passharsh,@Data6=Username,@Data7=Emailaddress, @Data8='Update',@Data9=(SELECT StationId FROM LnkStaffStation WHERE UserId = JSON_VALUE(@JsonObjectdata, '$.Userid') FOR JSON PATH) FROM Tenantusers a WHERE a.Userid=JSON_VALUE(@JsonObjectdata, '$.Userid')
		END
		ELSE
		BEGIN
		INSERT INTO Tenantusers(Tenantid,Firstname,Lastname,Phoneid,Phonenumber,Username,Emailaddress,Roleid,Passharsh,Passwords,LimitTypeId,LimitTypeValue,Isactive,Isdeleted,Loginstatus,Passwordresetdate,Createdby,Modifiedby,Lastlogin,Datemodified,Datecreated)
		...
		SELECT @UserId, StationId FROM OPENJSON (@JsonObjectdata, '$.SystemStation') WITH (StationId BIGINT)
		 ...
		 SELECT @Data1 = a.Userid,@Data2=a.Firstname +' '+ a.Lastname,@Data4=Passwords,@Data5=Passharsh,@Data6=Username,@Data7=Emailaddress, @Data8='Insert',@Data9=(SELECT StationId FROM LnkStaffStation WHERE UserId = @UserId FOR JSON PATH) FROM Tenantusers a WHERE a.Userid=@UserId
		END
		Set @RespMsg ='Success'
		Set @RespStat =0; 
		COMMIT TRANSACTION;
