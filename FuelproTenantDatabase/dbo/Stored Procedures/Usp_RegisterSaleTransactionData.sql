	...
	BEGIN
		...
		IF EXISTS(SELECT CustomerId FROM Customers WHERE CustomerId =(SELECT CustomerId FROM CustomerAgreements WHERE AgreementId=(SELECT AgreementId FROM CustomerAccount WHERE AccountId=JSON_VALUE(@JsonObjectdata, '$.AccountId'))) AND IsActive= 0)
	     ...
	     BEGIN
			 ...
			 BEGIN
			    --check customer Limit
				  ...
				  BEGIN
				   ...
				   return;
				  END
				  IF((SELECT COUNT(InvoiceId) FROM CreditInvoices WHERE CreditAgreementId =(SELECT CreditAgreementId FROM CreditAgreements WHERE AgreementId=(SELECT CA.AgreementId FROM CustomerAccount CA WHERE CA.AccountId =JSON_VALUE(@JsonObjectdata, '$.AccountId'))) AND IsPaid=0 AND dbo.getlocaldate()>=DueDate)>0)
				   BEGIN
			 ...
			 BEGIN
			  SET @FirstDateofMonth=(SELECT DATEADD(MONTH, DATEDIFF(MONTH, 0, dbo.getlocaldate()), 0));
			  SET @EndDateofMonth=(SELECT DATEADD(MONTH, DATEDIFF(MONTH, 0, dbo.getlocaldate()) + 1, 0));
		...
		BEGIN
		   SELECT @RedeemConvertionvalue=ConversionRatio FROM LRConversionMatrix  WHERE FromReward=(SELECT LRewardId FROM LRewards WHERE RewardName='Points') AND ToReward =(SELECT LRewardId FROM LRewards WHERE RewardName='Discount Voucher')
		   SELECT @TotalRewardAmount=SUM(LR.RewardAmount) FROM LRResults LR  WHERE LR.AccountId=JSON_VALUE(@JsonObjectdata, '$.AccountId');
		   IF((@TotalRewardAmount/@RedeemConvertionvalue) <@AmountBeingSpent)
			BEGIN
		...
		END
		BEGIN TRANSACTION;
		INSERT INTO FinanceTransactions(TransactionCode,FinanceTransactionTypeId,FinanceTransactionSubTypeId,CloseDayId,ParentId,Saledescription,SaleRefence,IsOnlineSale,AutomationRefence,Createdby,ActualDate,DateCreated)
		VALUES('TXN'+''+CONVERT(VARCHAR(70),NEXT VALUE FOR TransactionCodeSequence),(select TOP 1 FinanceTransactionTypeId from FinanceTransactionTypes),(select TOP 1 FinanceTransactionSubTypeId from FinanceTransactionSubTypes),
		(SELECT CloseDayId FROM CloseDays WHERE EndDate =(DATEADD(SECOND, -1, DATEADD(DAY, 1, CAST(CONVERT(DATE, dbo.getlocaldate()) AS DATETIME))))),(SELECT PeriodId FROM SystemPeriods WHERE Lastdateinperiod =(SELECT DATEADD(SECOND, -1, DATEADD(DAY, 1, CAST(EOMONTH(dbo.getlocaldate(), 0) AS datetime))))),'Sale',JSON_VALUE(@JsonObjectdata, '$.SaleReference'),JSON_VALUE(@JsonObjectdata, '$.IsSaleOnline'),@AutomationReference,
		JSON_VALUE(@JsonObjectdata, '$.CreatedbyId'),
		DATEADD(MILLISECOND,DATEPART(MILLISECOND, dbo.getlocaldate()),DATEADD(second, DATEPART(second, dbo.getlocaldate()), TRY_CAST(JSON_VALUE(@JsonObjectdata, '$.TransactionDate') AS datetime2(6)))),
		...
		END
		 ...
		 BEGIN
		   SELECT @RedeemConvertionvalue=ConversionRatio FROM LRConversionMatrix  WHERE FromReward=(SELECT LRewardId FROM LRewards WHERE RewardName='Points') AND ToReward =(SELECT LRewardId FROM LRewards WHERE RewardName='Discount Voucher')
			...
			INSERT INTO LRConversionDataInputs(FromReward,ToReward,StationId,AccountId,StaffId,IsProcessed,DateCreated,ConvertToAmount,FinanceTransactionId,Comments)
			(SELECT (SELECT LRewardId FROM LRewards WHERE RewardName='Points'),(SELECT LRewardId FROM LRewards WHERE RewardName='Discount Voucher'),JSON_VALUE(@JsonObjectdata, '$.StationId'),JSON_VALUE(@JsonObjectdata, '$.AccountId'),JSON_VALUE(@JsonObjectdata, '$.Userid'),1,TRY_CAST(JSON_VALUE(@JsonObjectdata, '$.TransactionDate') AS datetime2(6)),
			((SELECT TotalUsed FROM OPENJSON (@JsonObjectdata, '$.PaymentList') WITH (TotalUsed  DECIMAL(18,2)))/@RedeemConvertionvalue),@FinanceTransactionId,JSON_VALUE(@JsonObjectdata, '$.SaleReference'))
		 ...
		 JSON_VALUE(@JsonObjectdata, '$.CreatedbyId'),DATEADD(MILLISECOND,DATEPART(MILLISECOND, dbo.getlocaldate()),DATEADD(second, DATEPART(second, dbo.getlocaldate()), TRY_CAST(JSON_VALUE(@JsonObjectdata, '$.TransactionDate') AS datetime2(6)))),
		TRY_CAST(JSON_VALUE(@JsonObjectdata, '$.Datecreated') AS datetime2(6)));
