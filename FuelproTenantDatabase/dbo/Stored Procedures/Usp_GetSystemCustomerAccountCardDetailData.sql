...
--EXEC Usp_GetSystemCustomerAccountCardDetailData @SearchParameter='62453645634TEST',@StationId=1,@CustomerAccountDetailsJson=''
...
BEGIN
    ...
    BEGIN
           SELECT @RedeemConvertionvalue=ConversionRatio FROM LRConversionMatrix  WHERE FromReward=(SELECT LRewardId FROM LRewards WHERE RewardName='Points') AND ToReward =(SELECT LRewardId FROM LRewards WHERE RewardName='Discount Voucher')
			...
			INNER JOIN SystemStations STN ON STN.Tenantstationid = PLP.StationId 
			...
			--   END
			 ...
			 COALESCE(C.AmountPerDay,0) AS AmountPerDay,COALESCE(C.ConsecutiveTransTimeMin,0) AS ConsecutiveTransTimeMin,AG.AgreementId,AG.BillingBasis,SC.CardId,SC.CardUID AS CardUID,SC.CardSNO,0 AS HasDriverCode,CA.AccountId,
			...
			(
			 ...
			 (
				 SELECT LR.LRewardId,LW.RewardName,SUM(LR.RewardAmount) AS RewardValue,(@RedeemConvertionvalue / SUM(LR.RewardAmount)) AS RewardRedeemValue
				 FROM LRResults LR 
