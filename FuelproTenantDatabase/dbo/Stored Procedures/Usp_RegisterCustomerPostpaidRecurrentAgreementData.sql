...
CREATE PROCEDURE [dbo].[Usp_RegisterCustomerPostpaidRecurrentAgreementData]
   ...
   BEGIN
	DECLARE  
		...
		BEGIN
		 ...
		 IF((SELECT JSON_VALUE(@JsonObjectdata, '$.AgreementId'))='Monthly')
		 BEGIN
		  SET @BillingCycle= JSON_VALUE(@JsonObjectdata, '$.BillingCycle');
		 END 
		 ELSE
		 BEGIN
		  SET @BillingCycle= JSON_VALUE(@JsonObjectdata, '$.BillingDay');
		 END
			...
			SELECT
				...
				(SELECT AgreemettypeId FROM AgreementTypes WHERE Agreementtypename='Recurrent-'+@consumptiontype+'-Agreement'),
			...
			INSERT INTO PostpaidRecurentAgreements(AgreementId,AllowedDebt,Balance,BillingCycleType,BillingCycle,GracePeriod,StartDate,PreviousBillingDate,NextBillingDate,IsActive,IsDeleted,CreatedBy,ModifiedBy,DateCreated,DateModified)
			VALUES(@AgreementId,JSON_VALUE(@JsonObjectdata, '$.ConsumptionLimitValue'),JSON_VALUE(@JsonObjectdata, '$.ConsumptionLimitValue'),JSON_VALUE(@JsonObjectdata, '$.BillingCycleType'),
			@BillingCycle,JSON_VALUE(@JsonObjectdata, '$.GracePeriod'),JSON_VALUE(@JsonObjectdata, '$.StartDate'),JSON_VALUE(@JsonObjectdata, '$.StartDate'),JSON_VALUE(@JsonObjectdata, '$.StartDate'),1,0,JSON_VALUE(@JsonObjectdata, '$.CreatedbyId'),
