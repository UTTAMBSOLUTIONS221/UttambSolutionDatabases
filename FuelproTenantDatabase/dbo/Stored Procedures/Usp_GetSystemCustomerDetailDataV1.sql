...
--EXEC Usp_GetSystemCustomerDetailDataV1 @CustomerId=1,@CustomerDetails=''


CREATE PROCEDURE [dbo].[Usp_GetSystemCustomerDetailDataV1]
	@CustomerId BIGINT,
...
BEGIN
   BEGIN
	DECLARE 
		...
		SET @CustomerDetails =(
		SELECT 
         a.CustomerId, a.Designation,CASE WHEN Designation='Corporate' THEN a.Companyname ELSE a.Firstname+' '+ a.Lastname END AS Customername,a.CountryId,a.StationId,(SELECT TOP 1 CurrencySymbol FROM GeneralSettings A INNER JOIN SystemCurrencies B ON A.CurrencyId =B.CurrencyId) AS Currency,
		...
		ISNULL(a.Contractenddate,GETDATE()) AS Contractenddate,b.Sname AS Stationname,c.Countryname,d.Fullname AS Createdby,e.Fullname AS Modifiedby,a.Datecreated AS CustomerDatecreated,a.DateModified AS CustomerDateModified,a.NoOfTransactionPerDay,a.AmountPerDay,a.ConsecutiveTransTimeMin,
        (
          SELECT aa.AgreementId,(Select caa.AccountId from CustomerAccount caa WHERE ParentId= 0 AND caa.AgreementId = aa.AgreementId) AS AgreementAccountId,aa.CustomerId,aa.GroupingId,aa.AgreemettypeId,bb.Agreementtypename,aa.PriceListId,cc.PriceListname,aa.DiscountListId,dd.DiscountListname,aa.BillingBasis,aa.Descriptions,aa.AgreementDoc,
           aa.Notes,aa.Hasgroup,aa.HasOverdraft,aa.IsActive,aa.IsDeleted,ii.FullName AS Createdby,jj.FullName AS Modifiedby,aa.Datecreated,aa.Datemodified, ee.Paymentterms,gg.BillingCycleType,gg.BillingCycle,gg.GracePeriod AS RecurrentGracePeriod,gg.NextBillingDate,
		  ...
		  (SELECT ISNULL((SELECT SUM(CAST(amount as decimal(18,2))) FROM GLTransactions GL INNER JOIN  FinanceTransactions FT ON GL.FinanceTransactionId=FT.FinanceTransactionId WHERE FT.Saledescription in('Sale','SaleReverse') AND ChartofAccountId IN (SELECT ChartofAccountId FROM ChartofAccounts WHERE ChartofAccountname IN (SELECT CONVERT(VARCHAR(20),CA.AccountNumber) FROM CustomerAccount CA WHERE CA.AgreementId = aa.AgreementId  AND FT.ActualDate >= (SELECT DATEADD(MONTH, DATEDIFF(MONTH, 0, dbo.getlocaldate()), 0))))),0)) AS ConsumptiontoDate,
		  gg.AllowedDebt AS RecurrentAgreementLimit,
		  CASE WHEN hh.LimitTypename='Litres' THEN (gg.AllowedDebt-(SELECT ISNULL((SELECT SUM(CAST(TKL.Units as decimal(18,2))) FROM GLTransactions GL INNER JOIN  FinanceTransactions FT ON GL.FinanceTransactionId=FT.FinanceTransactionId INNER JOIN SystemTickets TKT ON TKT.FinanceTransactionId=FT.FinanceTransactionId INNER JOIN Ticketlines TKL ON TKL.TicketId=TKT.TicketId WHERE FT.Saledescription in('Sale','SaleReverse') AND ChartofAccountId IN (SELECT ChartofAccountId FROM ChartofAccounts WHERE ChartofAccountname IN (SELECT CONVERT(VARCHAR(20),CA.AccountNumber) FROM CustomerAccount CA WHERE CA.AgreementId = aa.AgreementId  AND FT.ActualDate >= (SELECT DATEADD(MONTH, DATEDIFF(MONTH, 0, dbo.getlocaldate()), 0))))),0))) ELSE  (gg.AllowedDebt-(SELECT ISNULL((SELECT SUM(CAST(amount as decimal(18,2))) FROM GLTransactions GL INNER JOIN  FinanceTransactions FT ON GL.FinanceTransactionId=FT.FinanceTransactionId WHERE FT.Saledescription in('Sale','SaleReverse') AND ChartofAccountId IN (SELECT ChartofAccountId FROM ChartofAccounts WHERE ChartofAccountname IN (SELECT CONVERT(VARCHAR(20),CA.AccountNumber) FROM CustomerAccount CA WHERE CA.AgreementId = aa.AgreementId  AND FT.ActualDate >= (SELECT DATEADD(MONTH, DATEDIFF(MONTH, 0, dbo.getlocaldate()), 0))))),0))) END AS  RecurrentAgreementBalance,
		  ( 
	       SELECT  ca.AccountId,ca.AgreementId,ca.AccountNumber,cc.CardSNO,ca.GroupingId,ca.ParentId,ca.CredittypeId,ct.Credittypename,ct.Credittypevalue,ca.LimitTypeId,clt.LimitTypename,clt.Limitkey,ca.ConsumptionLimit AS AccountLimit,ca.ConsumptionPeriod,
           (SELECT COUNT(*) FROM AccountEquipments WHERE AccountId=ca.AccountId) AS EquipmentAssigned,(SELECT COUNT(*) FROM AccountEmployee WHERE AccountId=ca.AccountId) AS UsersAssigned,ca.Isadminactive,ca.Iscustomeractive,ca.IsActive,ca.IsDeleted,CBA.Fullname AS CreatedBy,MB.Fullname AS ModifiedBy,ca.Datecreated,ca.Datemodified,
		   ...
		   CASE WHEN clt.LimitTypename='Litres' THEN
		   (SELECT ISNULL((SELECT SUM(Amount) FROM GLTransactions A WHERE ChartofAccountId IN (SELECT ChartofAccountId FROM ChartofAccounts WHERE ChartofAccountname =CONVERT(VARCHAR(20),ca.AccountNumber))),0) * -1)
		   ELSE
		   (SELECT ISNULL((SELECT SUM(Amount)  FROM GLTransactions WHERE ChartofAccountId IN (SELECT ChartofAccountId FROM ChartofAccounts WHERE ChartofAccountname =CONVERT(VARCHAR(20),ca.AccountNumber))),0) * -1)
