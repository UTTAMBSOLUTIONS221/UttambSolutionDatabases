...
BEGIN
   BEGIN
	DECLARE 
		...
		VALUES('TOP'+''+CONVERT(VARCHAR(70),NEXT VALUE FOR TransactionCodeSequence),(select TOP 1 FinanceTransactionTypeId from FinanceTransactionTypes),(select TOP 1 FinanceTransactionSubTypeId from FinanceTransactionSubTypes),
		(SELECT TOP 1 CloseDayId FROM CloseDays WHERE StartDate>=(CONVERT(datetime, CONVERT(date, dbo.getlocaldate())) + '00:00:00') AND EndDate<=(CONVERT(datetime, CONVERT(date, dbo.getlocaldate())) + '23:59:59') ),0,JSON_VALUE(@JsonObjectdata, '$.TransactionDescription'),JSON_VALUE(@JsonObjectdata, '$.TopupReference'),'TOP'+''+CONVERT(VARCHAR(70),NEXT VALUE FOR TransactionAutmationSequence),
