...
SELECT @PointAwardStatementDataDetailsJson = (
	     SELECT 
            ...
            (select Date,Description,Customer,Customeridnumber,Mask,IsNull(Equipment,'N/A') AS Equipment,Debit,Credit,AccountNumber,+ SUM(Balance) OVER (ORDER BY Date) AS Balance from #temptable FOR JSON PATH) AS PointsStatementData
