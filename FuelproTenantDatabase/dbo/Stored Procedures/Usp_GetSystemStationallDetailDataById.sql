...
BEGIN
   BEGIN
	DECLARE 
		...
		SELECT a.StationId,a.Sname,a.Tenantstationid,
			...
			,aa.NumberOfCalibrations,aa.IsActive,aa.IsDeleted,CB.Fullname AS CreatedBy,MB.Fullname AS ModifiedBy,aa.DateCreated,aa.DateModified
		  ...
		  WHERE aa.Stationid=a.Tenantstationid
		  FOR JSON PATH
		  ) AS StationTanks,
		  (SELECT bb.Pumpid,bb.Stationid,bb.Pumpname,bb.Pumpmodel,bb.Description,bb.Code,bb.IsDoubleSided,
		  bb.IsActive,bb.IsDeleted,CB.Fullname AS CreatedBy,MB.Fullname AS ModifiedBy,bb.DateCreated,bb.DateModified
		  ...
		  INNER JOIN SystemStaffs MB ON bb.ModifiedBy=MB.UserId
		   WHERE bb.Stationid=a.Tenantstationid
		   FOR JSON PATH
		  ) AS StationPumps
		  FROM SystemStations a
		  WHERE a.Tenantstationid=@StationId
