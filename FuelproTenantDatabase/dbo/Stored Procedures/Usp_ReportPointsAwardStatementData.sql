...
@CustomerAwardDataDetailsJson VARCHAR(MAX) OUTPUT
as
begin
SET NOCOUNT ON;  
    IF 1=0 BEGIN  
...
INSERT INTO #temptable
SELECT TRY_CAST(JSON_VALUE(@JsonObjectdata, '$.Startdate') AS datetime2(6))  AS Date,'Opening Balance' AS Description,'' AS Customer,'' AS Mask,'' AS Equipment,0  AS Debit,IsNull(@openingBalance,0) AS Credit,0 AS AccountNumber,IsNull(@openingBalance,0) AS Balance

-- Retrieve data from the temporary table and assign it to the output variable
    SELECT @CustomerAwardDataDetailsJson = (
	     SELECT 
		   ...
		   (SELECT Date,Description,Customer,Mask,Equipment,Debit,Credit,AccountNumber,+ SUM(Balance) OVER (ORDER BY Date) AS Balance FROM #temptable  FOR JSON PATH
			) AS CustomerAwardData
