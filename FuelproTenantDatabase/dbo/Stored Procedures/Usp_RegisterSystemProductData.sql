...
BEGIN
   BEGIN
	...
	BEGIN
		...
		BEGIN TRANSACTION;
		INSERT INTO SystemProduct(CategoryId,UomId,Productname,Productdescription,Createdby,Modifiedby,DateCreated,DateModified)
	    (SELECT JSON_VALUE(@JsonObjectdata, '$.CategoryId'),JSON_VALUE(@JsonObjectdata, '$.UomId'),JSON_VALUE(@JsonObjectdata, '$.Productname'),JSON_VALUE(@JsonObjectdata, '$.Productname'),
		...
		IF NOT EXISTS(SELECT PriceListId FROM PriceList WHERE IsDefault=1)
		BEGIN
			INSERT INTO PriceList(PriceListname,IsDefault,CreatedBy,ModifiedBy,DateCreated,DateModified)
			(SELECT 'Default Price',1,
			JSON_VALUE(@JsonObjectdata, '$.CreatedbyId'),JSON_VALUE(@JsonObjectdata, '$.ModifiedId'),
			CAST(JSON_VALUE(@JsonObjectdata, '$.Datecreated') AS datetime2),CAST(JSON_VALUE(@JsonObjectdata, '$.Datemodified')  AS datetime2))
		  ...
		  INSERT INTO PriceListprices(PriceListId,ProductvariationId,ProductPrice,StationId)
			SELECT @PriceListId, @ProductvariationId,JSON_VALUE(@JsonObjectdata, '$.Productprice'),StationId
