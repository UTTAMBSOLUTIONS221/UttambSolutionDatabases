...
BEGIN
   BEGIN
	...
	BEGIN
		...
		BEGIN TRANSACTION;
		INSERT INTO LSchemes(LSchemeName,StartDate,EndDate,CreatedBy,ModifiedBy,DateCreated,DateModified)
	    (SELECT JSON_VALUE(@JsonObjectdata, '$.LSchemeName'),CAST(JSON_VALUE(@JsonObjectdata, '$.StartDate')  AS datetime2),CAST(JSON_VALUE(@JsonObjectdata, '$.EndDate')  AS datetime2),
