...
BEGIN
   BEGIN
	DECLARE 
		...
		BEGIN
			IF EXISTS(SELECT CustomerId FROM Customers WHERE Emailaddress=JSON_VALUE(@JsonObjectdata, '$.Emailaddress'))
			BEGIN
				Select  1 as RespStatus, 'Similar Email Address exists!. Contact Admin!' as RespMessage;
				Return
			END
			IF EXISTS(SELECT CustomerId FROM Customers WHERE Phonenumber=JSON_VALUE(@JsonObjectdata, '$.Phonenumber'))
			BEGIN
				Select  1 as RespStatus, 'Similar Phonenumber exists!. Contact Admin!' as RespMessage;
				Return
			END
			IF EXISTS(SELECT CustomerId FROM Customers WHERE IDNumber=JSON_VALUE(@JsonObjectdata, '$.IDNumber'))
			BEGIN
		...
		BEGIN
		 ...
		 END
			 ...
			 INSERT INTO Customers(Firstname,Lastname,Companyname,Emailaddress,Phoneid,Phonenumber,Dob,Gender,IDNumber,Designation,Pin,Pinharsh,CompanyAddress,ReferenceNumber
		...
		Set @RespStat =0; 
		COMMIT TRANSACTION;
		Select @RespStat as RespStatus, @RespMsg as RespMessage, 'Insert' AS Data1, JSON_VALUE(@JsonObjectdata, '$.Companyname') AS Data2,JSON_VALUE(@JsonObjectdata, '$.Emailaddress') AS Data3, JSON_VALUE(@JsonObjectdata, '$.Emailaddress') AS Data4,'' AS Data5, '' AS Data6;
