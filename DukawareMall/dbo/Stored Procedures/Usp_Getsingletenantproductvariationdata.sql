CREATE PROCEDURE [dbo].[Usp_Getsingletenantproductvariationdata]
  @TenantproductId BIGINT,
  @SingleTenantProductVariations NVARCHAR(MAX) OUTPUT
AS
BEGIN
   BEGIN
	DECLARE 
		...
		BEGIN TRANSACTION;
				 SET @SingleTenantProductVariations = 
				 (
				 ...
				 FROM Tenantproduct a 
					...
					WHERE a.TenantproductId=@TenantproductId
		...
		SELECT  @RespStat as RespStatus, @RespMsg as RespMessage,@SingleTenantProductVariations AS SingleTenantProductVariations;
