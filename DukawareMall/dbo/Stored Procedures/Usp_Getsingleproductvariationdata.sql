CREATE PROCEDURE [dbo].[Usp_Getsingleproductvariationdata]
  @ProductId BIGINT,
  @SingleProductVariations NVARCHAR(MAX) OUTPUT
AS
BEGIN
   BEGIN
	DECLARE 
		...
		BEGIN TRANSACTION;
				 ...
				 FROM Tenantproduct a 
					...
					WHERE b.SystemproductId=@ProductId
		...
		SELECT  @RespStat as RespStatus, @RespMsg as RespMessage,@SingleProductVariations AS SingleProductVariations;
