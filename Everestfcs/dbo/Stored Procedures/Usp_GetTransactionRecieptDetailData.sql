    ...
    BEGIN
			...
			BEGIN
			   SELECT @CustomeragreementId=AG.AgreementId,@Agreementtypename=ATY.Agreementtypename,@Credittype=CTY.Credittypename,@AccountNumber=CA.AccountNumber  
					...
					--   END
					 ...
					 SET @FinanceTransactionDetailsJson = (SELECT @RespStat as RespStatus,@RespMsg AS RespMessage,N.Currencyname,A.Tenantid,A.TransactionCode,A.TransactionCode +''+CONVERT(VARCHAR(10),A.FinanceTransactionId) AS TransactionRefenece,
						...
						(SELECT ISNULL((SELECT ISNULL(LR.RewardName,'N/A') FROM LRADataInputs V INNER JOIN LRResults W ON V.LRADataInputId=W.LRADataInputId INNER JOIN LRewards LR ON W.LRewardId=LR.LRewardId WHERE V.FinanceTransactionId=A.FinanceTransactionId),'N/A')) AS RewardName,
						(SELECT ISNULL((SELECT ISNULL(W.RewardAmount,0) FROM LRADataInputs V INNER JOIN LRResults W ON V.LRADataInputId=W.LRADataInputId WHERE V.FinanceTransactionId=A.FinanceTransactionId),0)) AS CurrentPoints,
						(SELECT ISNULL(SUM(W.RewardAmount),0) FROM LRResults W WHERE W.AccountId=C.AccountId) AS CumulativePoints,
						J.Groupingname,A.SaleRefence,A.ActualDate,A.DateCreated,B.StaffId,L.Firstname+' '+L.Lastname AS Attendantname,U.Paymentmode,
						(
						...
						INNER JOIN SystemStations E ON B.StationId=E.StationId
						...
						INNER JOIN Tenantaccounts M ON A.Tenantid=M.Tenantid
						INNER JOIN SystemCountry N ON M.Countryid=N.CountryId
			...
			BEGIN
			     SET @FinanceTransactionDetailsJson = (SELECT @RespStat as RespStatus,@RespMsg AS RespMessage,A.TransactionCode,A.TransactionCode +''+CONVERT(VARCHAR(10),A.FinanceTransactionId) AS TransactionRefenece,
						...
						LEFT JOIN SystemStations E ON B.StationId=E.StationId
