...
BEGIN
    ...
    SELECT 
        ...
        s.Firstname+' '+s.Lastname AS Attendant,
    ...
    LEFT JOIN SystemStations F ON F.StationId = A.Stationid 
    ...
    LEFT JOIN SystemStaffs S ON S.UserId = A.StaffId
	INNER JOIN Paymentmodes U ON A.ModeofPayment=U.PaymentmodeId
    WHERE B.Tenantid=JSON_VALUE(@JsonObjectdata, '$.TenantId')  AND (JSON_VALUE(@JsonObjectdata, '$.Customer') < 1 OR E.CustomerId = JSON_VALUE(@JsonObjectdata, '$.Customer')) 
