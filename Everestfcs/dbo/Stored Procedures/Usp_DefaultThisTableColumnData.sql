...
BEGIN
   BEGIN
	DECLARE  
		...
		BEGIN TRANSACTION;
		BEGIN
		  SET @Sql1 = 'UPDATE '+ JSON_VALUE(@JsonObjectdata, '$.Tablename') +' SET '+ JSON_VALUE(@JsonObjectdata, '$.Columname') + ' = ' + CONVERT(NVARCHAR(MAX), 0) + ' Where ' + JSON_VALUE(@JsonObjectdata, '$.Columnidname') + ' = '  + JSON_VALUE(@JsonObjectdata, '$.Entryid')
