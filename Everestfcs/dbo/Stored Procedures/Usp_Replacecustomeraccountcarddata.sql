	...
	DECLARE @RespStat int = 0,
		...
		BEGIN TRANSACTION;
		 ...
		 BEGIN
			  ...
			  INSERT INTO Systemcard(TenantId,CardUID,CardSNO,PIN,PinHarsh,IsActive,IsDeleted,IsAssigned,IsReplaced,TagtypeId,Createdby,Modifiedby,Datecreated,Datemodified)
			  VALUES((SELECT a.Tenantid FROM SystemStaffs a  INNER JOIN Tenantaccounts b ON a.Tenantid=b.Tenantid WHERE a.Userid=JSON_VALUE(@JsonObjectdata, '$.Createdby')),@Accountnumber,CONVERT(VARCHAR(20),@Accountnumber)+'FUELPRO',JSON_VALUE(@JsonObjectdata, '$.Pin'),JSON_VALUE(@JsonObjectdata, '$.Pinharsh'),1,0,1,0,(SELECT TOP 1 TagtypeId FROM Systemcardtype WHERE TagTypename='V-Card'),JSON_VALUE(@JsonObjectdata, '$.Createdby'),JSON_VALUE(@JsonObjectdata, '$.Createdby'),CAST(JSON_VALUE(@JsonObjectdata, '$.Datecreated') AS datetime2),CAST(JSON_VALUE(@JsonObjectdata, '$.Datecreated')  AS datetime2))
			...
			INSERT INTO ReplacedCards(TenentId,OldCardid,NewCardid,Replacedby,Datecreated)
			VALUES((SELECT a.Tenantid FROM SystemStaffs a  INNER JOIN Tenantaccounts b ON a.Tenantid=b.Tenantid WHERE a.Userid=JSON_VALUE(@JsonObjectdata, '$.Createdby')),JSON_VALUE(@JsonObjectdata, '$.CardId'),@CardId,JSON_VALUE(@JsonObjectdata, '$.Createdby'),CAST(JSON_VALUE(@JsonObjectdata, '$.Datecreated') AS datetime2))
