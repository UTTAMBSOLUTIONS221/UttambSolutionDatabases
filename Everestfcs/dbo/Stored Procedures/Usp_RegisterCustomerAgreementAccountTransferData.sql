...
BEGIN
   BEGIN
	DECLARE 
		...
		VALUES(JSON_VALUE(@JsonObjectdata, '$.TenantId'),'TRANS'+''+CONVERT(VARCHAR(70),NEXT VALUE FOR TransactionCodeSequence),(select TOP 1 FinanceTransactionTypeId from FinanceTransactionTypes),(select TOP 1 FinanceTransactionSubTypeId from FinanceTransactionSubTypes),
		...
		(SELECT PeriodId FROM SystemPeriods WHERE Lastdateinperiod =(SELECT DATEADD(SECOND, -1, DATEADD(DAY, 1, CAST(EOMONTH((SELECT dbo.getlocaldate((SELECT c.Utcname FROM SystemStaffs a  INNER JOIN Tenantaccounts b ON a.Tenantid=b.Tenantid INNER JOIN SystemCountry c ON b.Countryid=c.CountryId WHERE a.Userid=JSON_VALUE(@JsonObjectdata, '$.CreatedbyId')))), 0) AS datetime))))),(SELECT c.Currencyname FROM SystemStaffs a  INNER JOIN Tenantaccounts b ON a.Tenantid=b.Tenantid INNER JOIN SystemCountry c ON b.Countryid=c.CountryId WHERE a.Userid=JSON_VALUE(@JsonObjectdata, '$.CreatedbyId')), CONVERT(decimal(10,4),(JSON_VALUE(@JsonObjectdata, '$.TransferAmount'))),CAST(JSON_VALUE(@JsonObjectdata, '$.Datecreated') AS datetime2),CAST(JSON_VALUE(@JsonObjectdata, '$.Datecreated') AS datetime2))
		...
		--Credit ACCOUNT TO
		INSERT INTO FinanceTransactions(Tenantid,TransactionCode,FinanceTransactionTypeId,FinanceTransactionSubTypeId,CloseDayId,ParentId,Saledescription,SaleRefence,AutomationRefence,Createdby,ActualDate,DateCreated)
		VALUES((SELECT a.Tenantid FROM SystemStaffs a  INNER JOIN Tenantaccounts b ON a.Tenantid=b.Tenantid WHERE a.Userid=JSON_VALUE(@JsonObjectdata, '$.CreatedbyId')),'TRANS'+''+CONVERT(VARCHAR(70),NEXT VALUE FOR TransactionCodeSequence),(select TOP 1 FinanceTransactionTypeId from FinanceTransactionTypes),(select TOP 1 FinanceTransactionSubTypeId from FinanceTransactionSubTypes),
		(SELECT  TOP 1 CloseDayId FROM CloseDays),0,JSON_VALUE(@JsonObjectdata, '$.TransactionDescription'),JSON_VALUE(@JsonObjectdata, '$.TransactionDescription'),'TRANS'+''+CONVERT(VARCHAR(70),NEXT VALUE FOR TransactionAutmationSequence),
		JSON_VALUE(@JsonObjectdata, '$.CreatedbyId'),TRY_CAST(JSON_VALUE(@JsonObjectdata, '$.Datecreated') AS datetime2(6)),TRY_CAST(JSON_VALUE(@JsonObjectdata, '$.Datecreated') AS datetime2(6)))
