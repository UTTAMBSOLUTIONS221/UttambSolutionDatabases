...
If(@Type = 3)
Select r.Sname as Text, r.StationId as Value From SystemStations r;
If(@Type = 4)
...
If(@Type =8)
Select r.TagTypename as Text, r.TagtypeId as Value From Systemcardtype r;
If(@Type =9)
...
Select r.RewardName as Text, r.LRewardId as Value From Lrewards r;
If(@Type =29)
Select r.Tenantname as Text, r.Tenantid as Value From TenantAccounts r;
If(@Type =37)
Select r.Paymentmode as Text, r.PaymentmodeId as Value,r.PaymentmodetypeId AS GroupId,p.Paymentmodetype AS Groupname From Paymentmodes r inner join Paymentmodetypes p on r.PaymentmodetypeId=p.PaymentmodetypeId where p.Paymentmodetype='BankCard';
END
