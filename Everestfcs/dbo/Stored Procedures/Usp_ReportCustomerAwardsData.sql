...
IF (JSON_VALUE(@JsonObjectdata, '$.Attendant') = 0 AND JSON_VALUE(@JsonObjectdata, '$.Station') = 0 AND JSON_VALUE(@JsonObjectdata, '$.Customer') = 0)
  ...
  SELECT SP.Productvariationname AS Product, PYMT.TotalUsed AS SaleValue, R.RewardAmount AS AwardValue, S.Sname AS Station, STF.FirstName+' '+ STF.LastName As Attendant, CASE WHEN CT.Credittypevalue = 0 THEN 'Prepaid' ELSE 'Postpaid' END AS AccountCreditType, R.DateCreated AS TransactionDate, COALESCE(SC.CardSNO,'N/A') AS Account, CA.AccountNumber, UPPER(RTRIM(LTRIM(ISNULL(C.FirstName, '') + ' ' + ISNULL(C.LastName, '') + ISNULL(C.CompanyName, '')))) AS Customer, 
  ...
  INNER JOIN SystemStations S ON S.StationId = TKT.StationId 
