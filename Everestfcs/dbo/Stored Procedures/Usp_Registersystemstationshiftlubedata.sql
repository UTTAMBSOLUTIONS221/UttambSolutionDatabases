CREATE PROCEDURE [dbo].[Usp_Registersystemstationshiftlubedata]
   ...
   BEGIN
	...
	BEGIN
		...
		BEGIN TRANSACTION;
			...
			USING (
			SELECT ShiftLubeLpgId,ShiftId,StockTypeId,AttendantId,ProductVariationId,OpeningUnits,ClosingUnits,UnitsSold,ProductPrice,UnitsTotal,Createdby,Modifiedby,DateCreated,DateModified
			FROM OPENJSON(@JsonObjectdata, '$.ShiftLubeReading')
			...
			)) AS source ON target.ShiftLubeLpgId = source.ShiftLubeLpgId
