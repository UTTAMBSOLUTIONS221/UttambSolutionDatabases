...
inner join SystemStations E on E.StationId = B.StationId
where A.Tenantid=JSON_VALUE(@JsonObjectdata, '$.TenantId')  AND (JSON_VALUE(@JsonObjectdata, '$.Customer')<1 OR P.CustomerId = JSON_VALUE(@JsonObjectdata, '$.Customer')) and A.ActualDate >= TRY_CAST(JSON_VALUE(@JsonObjectdata, '$.Startdate') AS datetime2(6)) and A.ActualDate < dateadd(day,1,TRY_CAST(JSON_VALUE(@JsonObjectdata, '$.Enddate') AS datetime2(6)))
...
left join SystemStations E on E.StationId = B.StationId
where A.Tenantid=JSON_VALUE(@JsonObjectdata, '$.TenantId')  AND (JSON_VALUE(@JsonObjectdata, '$.Customer')<1 OR AG.CustomerId = JSON_VALUE(@JsonObjectdata, '$.Customer')) 
...
BEGIN 
 ...
 where FinanceTransactions.Tenantid=JSON_VALUE(@JsonObjectdata, '$.TenantId')  AND ChartofAccountid  in (select ChartofAccountId from chartofAccounts where ChartofAccountname in
...
BEGIN
    set @openingBalance  = (
	select sum(amount) from GLTransactions  LEFT JOIN FinanceTransactions ON FinanceTransactions.FinanceTransactionId=GLTransactions.FinanceTransactionId 
	where FinanceTransactions.Tenantid=JSON_VALUE(@JsonObjectdata, '$.TenantId')  AND  ChartofAccountid  = (select ChartofAccountId from chartofAccounts where ChartofAccountname = (SELECT CONVERT(VARCHAR, AccountNumber) FROM CustomerAccount WHERE AccountId=JSON_VALUE(@JsonObjectdata, '$.Account')))
	and FinanceTransactions.ActualDate <= TRY_CAST(JSON_VALUE(@JsonObjectdata, '$.Startdate') AS datetime2(6)) );
