...
BEGIN
   BEGIN
	DECLARE @RespStat int = 0,
			...
			@LRConversionDataInputId BIGINT,
			@NewLRConversionDataInputId BIGINT,
	...
	BEGIN
		...
		SELECT TenantId,'TXN'+''+CONVERT(VARCHAR(70),NEXT VALUE FOR TransactionCodeSequence),FinanceTransactionTypeId,FinanceTransactionSubTypeId, 
		...
		BEGIN
		    SET @NewLRConversionDataInputId=(SELECT LRConversionDataInputId FROM LRConversionDataInputs WHERE FinanceTransactionId = JSON_VALUE(@JsonObjectdata, '$.FinanceTransactionId'));
			...
			SELECT @FinanceTransactionId, ToReward, FromReward, StationId, AccountNumber, StaffId, IsProcessed, DateCreated, -1*(ConvertToAmount), Comments FROM LRConversionDataInputs WHERE FinanceTransactionId = JSON_VALUE(@JsonObjectdata, '$.FinanceTransactionId')
			SET @LRConversionDataInputId=SCOPE_IDENTITY();
			...
			(SELECT  TOP 1 AccountId, LRewardId, LTransactionTypeId, LRADataInputId, @LRConversionDataInputId, -1*(RewardAmount) AS RewardAmount, IsActive, IsDeleted, Createdby, Modifiedby, ActualDateCreated, TRY_CAST(JSON_VALUE(@JsonObjectdata, '$.Datecreated') AS datetime2(6)), TRY_CAST(JSON_VALUE(@JsonObjectdata, '$.Datecreated') AS datetime2(6)) 
			FROM LRResults 
			WHERE LRConversionDataInputId =@NewLRConversionDataInputId AND LTransactionTypeId = (SELECT LtransactionTypeId FROM LtransactionTypes WHERE LtransactionTypeName = 'Redeem'))
			INSERT INTO LRResults(AccountId, LRewardId, LTransactionTypeId, LRADataInputId, LRConversionDataInputId, RewardAmount, IsActive, IsDeleted, Createdby, Modifiedby, ActualDateCreated, DateCreated, DateModified)
			(SELECT AccountId, LRewardId, LTransactionTypeId, LRADataInputId, @LRConversionDataInputId, -1*(RewardAmount) AS RewardAmount, IsActive, IsDeleted, Createdby, Modifiedby, ActualDateCreated, TRY_CAST(JSON_VALUE(@JsonObjectdata, '$.Datecreated') AS datetime2(6)), TRY_CAST(JSON_VALUE(@JsonObjectdata, '$.Datecreated') AS datetime2(6)) 
			FROM LRResults WHERE LRConversionDataInputId =@NewLRConversionDataInputId AND LTransactionTypeId = (SELECT LtransactionTypeId FROM LtransactionTypes WHERE LtransactionTypeName = 'Conversion'))
			INSERT INTO LRResults(AccountId, LRewardId, LTransactionTypeId, LRADataInputId, LRConversionDataInputId, RewardAmount, IsActive, IsDeleted, Createdby, Modifiedby, ActualDateCreated, DateCreated, DateModified)
			(SELECT AccountId, LRewardId, LTransactionTypeId, LRADataInputId, @LRConversionDataInputId, -1*(RewardAmount) AS RewardAmount, IsActive, IsDeleted, Createdby, Modifiedby, ActualDateCreated, TRY_CAST(JSON_VALUE(@JsonObjectdata, '$.Datecreated') AS datetime2(6)), TRY_CAST(JSON_VALUE(@JsonObjectdata, '$.Datecreated') AS datetime2(6)) 
			FROM LRResults 
			WHERE LRConversionDataInputId =@NewLRConversionDataInputId AND LTransactionTypeId = (SELECT LtransactionTypeId FROM LtransactionTypes WHERE LtransactionTypeName = 'RewardUtilization'))
		END
