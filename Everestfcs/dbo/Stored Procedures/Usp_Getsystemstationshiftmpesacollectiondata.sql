CREATE PROCEDURE [dbo].[Usp_Getsystemstationshiftmpesacollectiondata]
@ShiftId BIGINT,
@start INT,
@length INT,
...
SET @DataTableData=(
		...
		WHERE SVV.VoucherType='ShiftMpesaCollection'  AND SVV.ShiftId=SCC.ShiftId 
		ORDER BY SVV.ShiftVoucherId DESC 
		OFFSET @start ROWS FETCH NEXT @length ROWS ONLY
		FOR JSON PATH
		) AS DataTableData
		FROM ShiftVouchers SCC  
		WHERE SCC.VoucherType='ShiftMpesaCollection' AND SCC.ShiftId=@ShiftId 
