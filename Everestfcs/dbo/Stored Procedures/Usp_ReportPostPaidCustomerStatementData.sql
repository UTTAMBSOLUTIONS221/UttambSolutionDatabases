...
where A.Tenantid=JSON_VALUE(@JsonObjectdata, '$.TenantId') AND A.ActualDate >= TRY_CAST(JSON_VALUE(@JsonObjectdata, '$.Startdate') AS datetime2(6)) and A.ActualDate <= dateadd(day,1,TRY_CAST(JSON_VALUE(@JsonObjectdata, '$.Enddate') AS datetime2(6)))  and (JSON_VALUE(@JsonObjectdata, '$.Customer')=-1 OR K.CustomerId = JSON_VALUE(@JsonObjectdata, '$.Customer'))
...
where FT.Tenantid=JSON_VALUE(@JsonObjectdata, '$.TenantId')  AND FT.ActualDate >= TRY_CAST(JSON_VALUE(@JsonObjectdata, '$.Startdate') AS datetime2(6)) and FT.ActualDate <= dateadd(day,1,TRY_CAST(JSON_VALUE(@JsonObjectdata, '$.Enddate') AS datetime2(6)))  and (JSON_VALUE(@JsonObjectdata, '$.Customer')=-1 OR CA.CustomerId = JSON_VALUE(@JsonObjectdata, '$.Customer'))
...
AND FT.Tenantid=JSON_VALUE(@JsonObjectdata, '$.TenantId')  AND FT.ActualDate <= TRY_CAST(JSON_VALUE(@JsonObjectdata, '$.Startdate') AS datetime2(6)) AND PYMT.PaymentModeId = (SELECT TOP 1 PaymentmodeId FROM PaymentModes where Paymentmode = 'Card')) * -1)
...
Where FT.Tenantid=JSON_VALUE(@JsonObjectdata, '$.TenantId')  AND AgreementId = JSON_VALUE(@JsonObjectdata, '$.Agreement') 
