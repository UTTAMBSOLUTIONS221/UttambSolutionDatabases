...
BEGIN
   BEGIN
	DECLARE 
		...
		VALUES(JSON_VALUE(@JsonObjectdata, '$.TenantId'),'PAY'+''+CONVERT(VARCHAR(70),NEXT VALUE FOR TransactionCodeSequence),(select TOP 1 FinanceTransactionTypeId from FinanceTransactionTypes),(select TOP 1 FinanceTransactionSubTypeId from FinanceTransactionSubTypes),
		...
		(SELECT TOP 1 c.Currencyname FROM SystemStaffs a  INNER JOIN Tenantaccounts b ON a.Tenantid=b.Tenantid INNER JOIN SystemCountry c ON b.Countryid=c.CountryId WHERE a.Tenantid=JSON_VALUE(@JsonObjectdata, '$.TenantId')),-1 * CONVERT(decimal(10,4),(JSON_VALUE(@JsonObjectdata, '$.Amount'))),CAST(JSON_VALUE(@JsonObjectdata, '$.TransactionDate') AS datetime2),CAST(JSON_VALUE(@JsonObjectdata, '$.DateCreated') AS datetime2))
		...
		BEGIN
			...
			BEGIN
			 UPDATE CreditInvoices SET IsPaid =1,PayStatus='PAID', PaidAmount=@CurrentInvoiceAmount,Balance=0 WHERE InvoiceId =@CurrentInvoiceID
			  ...
			  SELECT @CustomerPaymentId,@CurrentInvoiceID,@CurrentInvoiceAmount,(SELECT dbo.getlocaldate((SELECT TOP 1 c.Utcname FROM SystemStaffs a  INNER JOIN Tenantaccounts b ON a.Tenantid=b.Tenantid INNER JOIN SystemCountry c ON b.Countryid=c.CountryId WHERE a.Tenantid=JSON_VALUE(@JsonObjectdata, '$.TenantId'))))
