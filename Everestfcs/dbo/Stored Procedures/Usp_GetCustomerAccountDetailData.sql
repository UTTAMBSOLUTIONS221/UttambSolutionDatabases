   ...
   BEGIN
	DECLARE 
	 ...
	 BEGIN
		...
		SELECT
		C.CustomerId,A.AccountId,CASE WHEN C.Designation='Corporate' THEN C.Companyname ELSE C.Firstname+' '+ C.Lastname END AS Customername,C.Emailaddress,A.AccountNumber,I.CardSNO,I.CardUID,A.ConsumptionLimit,SC.Currencyname AS Currency,
		...
		(
		  ...
		  aa.Chequeno,aa.Bankaccno,aa.Drawerbank,aa.Payeebank,aa.Branchdeposited,aa.Depositslip,aa.Createdby,dd.Firstname+' '+dd.Lastname AS Fullname,aa.DateCreated
		...
		INNER JOIN Tenantaccounts TA ON C.Tenantid=TA.Tenantid
	     INNER JOIN SystemCountry SC ON TA.Countryid=SC.CountryId
