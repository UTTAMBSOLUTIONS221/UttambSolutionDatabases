...
BEGIN
   BEGIN
	DECLARE 
		...
		(SELECT JSON_VALUE(@JsonObjectdata, '$.Tenantid'),JSON_VALUE(@JsonObjectdata, '$.Firstname'),JSON_VALUE(@JsonObjectdata, '$.Lastname'),JSON_VALUE(@JsonObjectdata, '$.Emailaddress'),JSON_VALUE(@JsonObjectdata, '$.Roleid'),
		...
		JSON_VALUE(@JsonObjectdata, '$.Passwords'),JSON_VALUE(@JsonObjectdata, '$.Passwordharsh'),1,0,0,1,1,GETDATE(),JSON_VALUE(@JsonObjectdata, '$.Extra'),JSON_VALUE(@JsonObjectdata, '$.Extra1'),JSON_VALUE(@JsonObjectdata, '$.Extra2'),JSON_VALUE(@JsonObjectdata, '$.Extra3'),
		...
		JSON_VALUE(@JsonObjectdata, '$.Extra9'),0,2,JSON_VALUE(@JsonObjectdata, '$.Createdby'),JSON_VALUE(@JsonObjectdata, '$.Modifiedby'),CAST(JSON_VALUE(@JsonObjectdata, '$.Datecreated') AS datetime2),CAST(JSON_VALUE(@JsonObjectdata, '$.Datemodified') AS datetime2))
		...
		Set @RespMsg ='Staff Updated Successfully.'
		Set @RespStat =0; 
		END
